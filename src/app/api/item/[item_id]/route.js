...
// SOFT DELETE:
// Instead of removing the document with deleteOne(), we UPDATE its
// "status" field to "DELETED". The item still physically exists in
// MongoDB, but the GET /api/item list route filters status: "ACTIVE"
// so it no longer shows up to the user.
export async function DELETE(request, { params }) {
  ...
  try {
    ...
    const updateOk = Number(updateResult.modifiedCount) > 0;
